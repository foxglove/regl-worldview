...
type Story = (setTestData: (arg0: any) => void, state: any) => React.ReactElement<React.ComponentProps<any>, any>;
type Assertion = (getTestData: () => any, setState: (arg0: any) => void) => Promise<void>;
type AssertionTest = {
  story: Story;
...
export function assertionTest({
  story,
  assertions
}: AssertionTest): () => React.ReactElement<React.ComponentProps<any>, any> {
  ...
  function Component() {
    ...
    useLayoutEffect(() => {
      ...
      const resultPromise = Promise.race([assertionPromise(), errorPromise, consoleErrorPromise]).catch(err => setError(err)).then(() => {
