...
function ScrollTo({
  ...
  if (typeof children === "string") {
    linkStr = children.replace(/[\W_]+/g, " ") // only keep letters, numbers and spaces
  ...
  return <div ref={wrapperRef}>
      {linkStr ? <Link style={{
      textDecoration: "none"
    }} to={disableAnchorLink ? match.path : `${match.path}/${linkStr}`}>
          <Tag>{children}</Tag>
        </Link> : <Tag>{children}</Tag>}
    </div>;
}
...
const components = {
  ...
  h2: props => <ScrollToWithRouter component="h2" {...props} />,
  // eslint-disable-line react/display-name
  h3: props => <ScrollToWithRouter component="h3" {...props} />,
  // eslint-disable-line react/display-name
