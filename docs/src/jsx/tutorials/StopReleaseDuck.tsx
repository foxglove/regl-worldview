...
function Example() {
  ...
  const obstacleMarkers = Array.from(clickedObjectIds).map(({
    ...
    return {
      ...
      scale: {
        ...
        y: 6,
        z: 6
      } // scale up a little so it's bigger than the spheres
  ...
  useEffect(() => {
    ...
    if (!shouldStopDuck && clickedObjectIds.some(({
      id,
      instanceIndex
    }) => id + instanceIndex === duckPositionId)) {
  ...
  return <Worldview defaultCameraState={{
    ...
    thetaOffset: -Math.PI / 2 // rotate the camera so the duck is facing right
      ...
      setClickedObjectIds([...clickedObjectIds, {
        id: objects[0].object.id,
        instanceIndex: objects[0].instanceIndex
      }]);
    }}>
        {[sphereMarker]}
      </Spheres>
      ...
      const newClickedObjectIds = clickedObjectIds.filter(({
        id,
        instanceIndex
      }) => id + instanceIndex !== objects[0].object.clickedObjectId);
