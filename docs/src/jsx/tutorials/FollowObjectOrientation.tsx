...
function Example() {
  ...
  new Array(steps).fill().map((_, idx) => [// generate x, y, z coordinates based on trefoil equation
  Math.sin(idx * scale) + 2 * Math.sin(2 * idx * scale), Math.cos(idx * scale) - 2 * Math.cos(2 * idx * scale), -Math.sin(3 * idx * scale)]).forEach(([x, y, z], idx) => {
    // add individual point and color to the single sphere object
    sphereMarker.colors.push(numberToColor(idx, steps));
    sphereMarker.points.push({
      x: x * 20,
      y: y * 20,
      z: z * 20
    });
  ...
  const tangentVecO = [Math.cos(count * scale) + 4 * Math.cos(2 * count * scale), -Math.sin(count * scale) + 4 * Math.sin(2 * count * scale), -3 * Math.cos(3 * count * scale)];
  ...
  quat.rotateY(duckOrientation, duckOrientation, -Math.atan(tangentVecO[2] / Math.hypot(tangentVecO[0], tangentVecO[1])));
  quat.rotateZ(duckOrientation, duckOrientation, -Math.PI / 2);
  return <Worldview cameraState={{
