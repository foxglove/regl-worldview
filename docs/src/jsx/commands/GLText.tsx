...
const ALPHABET = [].concat( // all characters to pick from
...[[0x30, 0x39], [0x41, 0x5a], [0x410, 0x42f], [0x3041, 0x3096]].map(([start, end]) => new Array(end - start + 1).fill().map((_, i) => String.fromCodePoint(start + i))));
...
class MatrixText {
  ...
  step() {
    this._cols = this._cols.map(col => {
      ...
      const firstKeepIdx = col.chars.findIndex(({
        age
      }) => age < FADE);
...
function Example() {
  ...
  return <Worldview cameraState={cameraState} onCameraStateChange={newState => setCameraState(oldState => ({ ...oldState,
    ...newState,
