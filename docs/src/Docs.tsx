  ...
  return <React.Fragment>
      <SideBar>
        ...
        <Navigation isMobileNavOpen={isMobileNavOpen}>
          {routes.map(route => <Section key={route.path}>
              {route.name && <div>{route.name}</div>}
              <ul>
                {route.subRoutes.map(subRoute => <li key={subRoute.path}>
                    <NavLink to={`${route.path}${subRoute.path}`} onClick={() => toggleMobileNav(false)} activeClassName="active">
      ...
      <MainContainer>
        <Main>
          ...
          {routes.map(route => <React.Fragment key={route.path}>
              <Route exact path={route.path} render={() => <Redirect to={`${route.path}${route.subRoutes[0].path}`} />} />
              {route.subRoutes.map(subRoute => <div className="markdown-body" key={subRoute.path}>
                  <Route exact={subRoute.exact} path={`${route.path}${subRoute.path}`} component={componentList[subRoute.main] || DefaultMain} />
                </div>)}
            </React.Fragment>)}
        </Main>
      </MainContainer>
