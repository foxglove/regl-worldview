...
export type CommonCommandProps = {
  ...
  [key: MouseEventEnum]: ComponentMouseHandler;
};
type Props<T> = CommonCommandProps & {
  children?: T[] | T;
  // Deprecated, but here for backwards compatibility
  drawProps?: T[];
  reglCommand: RawCommand<T>;
};
export type CommandProps<T> = Props<T>; // Component to dispatch children (for drawing) and hitmap props and a reglCommand to the render loop to render with regl.

export default class Command<T> extends React.Component<Props<T>> {
  context: WorldviewContextType | null | undefined;
