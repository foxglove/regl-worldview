...
export default class HitmapObjectIdManager {
  _objectsByObjectHitmapIdMap: Record<ObjectHitmapId, Record<string, any>> = {};
  _commandsByObjectMap: Map<Record<string, any>, CommandType> = new Map();
  _nextObjectHitmapId = 1;
  _instanceIndexByObjectHitmapIdMap: Record<ObjectHitmapId, number> = {};
  assignNextColors = (command: CommandType, object: Record<string, any>, count: number): Vec4[] => {
    if (count < 1) {
