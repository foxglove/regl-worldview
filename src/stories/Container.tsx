...
export default class Container extends React.Component<Props> {
  ...
  render() {
    ...
    const worldviewCamStateInfo = Object.keys(worldviewCamState).map(key => `${key}: ${worldviewCamState[key]}`).join("\n");
    return <div style={{
      position: "relative",
      width: "100%",
      height: "100%"
    }}>
        {!this.props.hideState && <div style={{
        position: "absolute",
        pointerEvents: "none",
        width: "100%",
        bottom: "8px",
        ...
        <Worldview {...this.props} defaultCameraState={undefined} cameraState={worldviewCamState} onCameraStateChange={this.onCameraStateChange}>
          {this.props.children}
        </Worldview>
      </div>;
  }
