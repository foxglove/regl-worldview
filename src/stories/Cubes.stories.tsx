...
class DynamicCubes extends React.Component<any, any> {
  ...
  render() {
    ...
    return <Container cameraState={DEFAULT_CAMERA_STATE} onClick={this.onContainerClick}>
        <div style={{
        position: "absolute",
        top: 30,
        left: 30
...
storiesOf("Worldview/Cubes", module).add("<Cubes> - single", withRange(range => {
...
})).add("<Cubes> - dynamic hitmap", withRange(range => {
  return <DynamicCubes range={range} />;
})).add("<Cubes> - instanced", withRange(range => {
  const cube = {
    id: 1,
    pose: {
  ...
  return <Container cameraState={instancedCameraState}>
          <Cubes>{[cube]}</Cubes>
        </Container>;
})).add("<Cubes> - per-instance colors", withRange(range => {
  const points = buildMatrix(20, 20, 20, 20);
  window.cubeColors = window.cubeColors || points.map(() => ({
    r: rng(),
    g: rng(),
    b: rng(),
    a: 1
