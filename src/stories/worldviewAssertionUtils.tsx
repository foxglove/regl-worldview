 ...
 */
export function generateNonInstancedClickAssertions<Type>(commandName: string, CommandInstance: ComponentType<CommonCommandProps & {
  children: Array<Type>;
}>, renderedObjects: Array<Type>, overrideOptions?: {
  overrideExpectedSingleObjects?: any;
  overrideExpectedMultipleObjects?: any;
}): Array<{
  name: string;
  story: () => React.ReactElement<React.ComponentProps<any>, any>;
}> {
  const options = overrideOptions || {};
  return [{
    name: `Clicks on a single ${commandName} - worldview event handler`,
    story: assertionTest({
      story: setTestData => <WorldviewWrapper onClick={(_, {
        objects
      }) => setTestData(objects)}>
      ...
      assertions: async getTestData => {
        ...
        expect(result).toEqual(options.overrideExpectedSingleObjects ? options.overrideExpectedSingleObjects : [{
    ...
    story: assertionTest({
      story: setTestData => <WorldviewWrapper>
            <CommandInstance onClick={(_, {
          objects
        }) => setTestData(objects)}>{renderedObjects}</CommandInstance>
          </WorldviewWrapper>,
      assertions: async getTestData => {
        await clickAtOrigin();
        const result = getTestData();
    ...
    story: assertionTest({
      story: setTestData => <WorldviewWrapper onClick={(_, {
        objects
      }) => setTestData(objects)} enableStackedObjectEvents>
            <CommandInstance>{renderedObjects}</CommandInstance>
          </WorldviewWrapper>,
      assertions: async getTestData => {
        await clickAtOrigin();
        const result = getTestData();
        expect(result).toEqual(options.overrideExpectedMultipleObjects ? options.overrideExpectedMultipleObjects : [{
          object: renderedObjects[0],
          instanceIndex: undefined
        }, {
          object: renderedObjects[1],
          instanceIndex: undefined
        }]);
      }
    })
 ...
 */
export function generateInstancedClickAssertions<Type>(commandName: string, CommandInstance: ComponentType<CommonCommandProps & {
  children: Array<Type>;
}>, renderedObject: Type): Array<{
  name: string;
  story: () => React.ReactElement<React.ComponentProps<any>, any>;
}> {
  return [{
    ...
    story: assertionTest({
      story: setTestData => <WorldviewWrapper>
            <CommandInstance onClick={(_, {
          objects
        }) => setTestData(objects)}>{[renderedObject]}</CommandInstance>
          </WorldviewWrapper>,
      assertions: async getTestData => {
        await clickAtOrigin();
        const result = getTestData();
