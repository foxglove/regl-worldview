...
stories.add(`Clicks on a single GLTFScene object - worldview event handler`, assertionTest({
  story: setTestData => {
    const duckModel = require("~/common/fixtures/Duck.glb");

    return <WorldviewWrapper onClick={(_, {
      objects
    }) => setTestData(objects)}>
  ...
  assertions: async getTestData => {
    ...
    expect(result).toEqual([{
      object: firstModelPosition,
      instanceIndex: undefined
    }]);
  }
})).add(`Clicks on a GLTFSCene object with an object behind it - worldview event handler`, assertionTest({
  story: setTestData => {
    const duckModel = require("~/common/fixtures/Duck.glb");

    let testData = [];
    return <WorldviewWrapper enableStackedObjectEvents>
            <GLTFScene onClick={(_, {
        objects
      }) => {
