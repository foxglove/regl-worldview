...
storiesOf("Integration/Grid", module)
  .add(
    "Clicks on a Grid - worldview event handler", // The grid does have individual event handlers, but they could still be clicked on by worldview event handlers.
    assertionTest({
      story: (setTestData) => (
  ...
  .add(
    "Clicks on a Grid with an object behind it - worldview event handler", // Make sure that the axis removes itself from the `excludedObjects`.
    assertionTest({
      story: (setTestData) => (
        <WorldviewWrapper
