...
generateNonInstancedClickAssertions<TriangleList>("Triangle with onlyRenderInHitmap=true", Triangles, twoTrianglesInARow.map(triangle => ({ ...triangle,
  onlyRenderInHitmap: true
}))).forEach(({
  name,
  story
}) => stories.add(name, story));
generateInstancedClickAssertions<TriangleList>("Triangle", Triangles, instancedTriangles).forEach(({
  name,
  story
}) => stories.add(name, story));